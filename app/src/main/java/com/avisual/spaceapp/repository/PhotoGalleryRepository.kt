...
class PhotoGalleryRepository(database: Db) {
    ...
    suspend fun deletePhoto(photoGallery: PhotoGallery) = withContext(Dispatchers.IO) {
        photoGalleryDao.delete(photoGallery)
    }
    ...
    suspend fun getFindByNasaId(nasaId: String): PhotoGallery? = withContext(Dispatchers.IO) {
